...
Okapi_TopKWords = function(q, k1, b, top){
        ...
        mapFunction = function() {
                        for (var idx=0; idx<this.words.length; idx++){
                            emit(this.words[idx].word, { count: 1, stf: [{tf: this.words[idx].tf, docLen: this.document_dimension.lemmaTextLength}] });
                        }
                    }
        ...
        db.document_facts.aggregate([ { $match: q}, { $group: { _id: null, avgDL: { $avg: "$document_dimension.lemmaTextLength" }}}]).forEach(function(elem){avgDL = elem.avgDL;})

        db.document_facts.mapReduce(mapFunction, reduceFunction, {query: q, out: "vocabulary", scope: {noDocs: noDocs, k1: k1, b: b, avgDL: avgDL}, finalize: finalizeFunction});
