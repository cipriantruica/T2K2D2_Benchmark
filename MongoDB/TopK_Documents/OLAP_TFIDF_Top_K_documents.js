OLAP_TFIDF_MR_TopKDocs = function(q_noDocs, q_db, words, k){
    ...
    mapFunctionTFIDF = function() {
                        for (var idx=0; idx<this.words.length; idx++){
                            if (words.indexOf(this.words[idx].word) != -1 ){
                                emit(this.document_dimension.id_document, this.words[idx].tf * idf[this.words[idx].word]);
                            }
                        }
                    };   

    reduceFunctionTFIDF = function(key, values) {
                            return Array.sum(values);;
                    };

    db.document_facts.mapReduce(mapFunctionTFIDF, reduceFunctionTFIDF, {query: q_db, out: "vocabulary", scope: {noDocs: noDocs, words:words, idf: idfs_word}});

    var items = db.vocabulary.find().sort({value: -1, _id: 1}).limit(k).addOption(DBQuery.Option.noTimeout);
        // while(items.hasNext()){
        //     var item = items.next();
...
//testing function
OLAP_TFIDF_MR_testingFunction = function(){
    ...
    for(var idx=0; idx<words.length; idx++){
        ...
        for (i=0; i<10; i++){
            ...
            // print('Q1 - male');
            startTime = new Date();
            OLAP_TFIDF_MR_TopKDocs(q1a_noDocs, q1a_db, words[idx], k);
            endTime = new Date();
            times['male']['q1'].push(endTime - startTime);
            db.vocabulary.drop();
            // print('Q1 - female');
            startTime = new Date();
            OLAP_TFIDF_MR_TopKDocs(q1b_noDocs, q1b_db, words[idx], k);        
            endTime = new Date();
            times['female']['q1'].push(endTime - startTime);
            db.vocabulary.drop();
            // print('Q2 - male');
            startTime = new Date();
            OLAP_TFIDF_MR_TopKDocs(q2a_noDocs, q2a_db, words[idx], k);
            endTime = new Date();
            times['male']['q2'].push(endTime - startTime);
            db.vocabulary.drop();
            // print('Q2 - female');
            startTime = new Date();
            OLAP_TFIDF_MR_TopKDocs(q2b_noDocs, q2b_db, words[idx], k);
            endTime = new Date();
            times['female']['q2'].push(endTime - startTime);
            db.vocabulary.drop();
            // print('Q3 - male');
            startTime = new Date();
            OLAP_TFIDF_MR_TopKDocs(q3a_noDocs, q3a_db, words[idx], k);
            endTime = new Date();
            times['male']['q3'].push(endTime - startTime);
            db.vocabulary.drop();
            // print('Q3 - female');
            startTime = new Date();
            OLAP_TFIDF_MR_TopKDocs(q3b_noDocs, q3b_db, words[idx], k);
            endTime = new Date();
            times['female']['q3'].push(endTime - startTime);
            db.vocabulary.drop();
            // print('Q4 - male');
            startTime = new Date();
            OLAP_TFIDF_MR_TopKDocs(q4a_noDocs, q4a_db, words[idx], k);
            endTime = new Date();
            times['male']['q4'].push(endTime - startTime);
            db.vocabulary.drop();
            // print('Q4 - female');
            startTime = new Date();
            OLAP_TFIDF_MR_TopKDocs(q4b_noDocs, q4b_db, words[idx], k);
            endTime = new Date();
            times['female']['q4'].push(endTime - startTime);
            db.vocabulary.drop();
        }
        print('OLAP TFIDF MR', words[idx].length, "words");
        print('Q1 - male');
        print(times['male']['q1']);
        print('Q1 - female');
